...
// Rechercher un fichier image lorsque la fenêtre centrale reçoit un clic souris
function upload() {
    uploadBtn.click();
}

// Permettre les boutons
...
// Désactiver tous les boutons en charge
enableBtns(false, false);

// Téléverser un fichier
uploadBtn.addEventListener("change", () => {
    // Effacer les filtres/effets précédents
    revertBtn.click();
    ...
    if (file) {
        if (['image/jpeg', 'image/gif', 'image/webp', 'image/png'].includes(file.type)) {
            // Définir le nom et le type du fichier
            fileName = file.name;
            ...
            // Voir la fenêtre d'alerte
            dwarning("Format de fichier non pris en charge");
            return;
        }

        // Lire les données comme URL
        reader.readAsDataURL(file);
    }

    // Ajouter une image à la toile
...
// Ajoutez une info-bulle bootstrap à chaque balise qui a une classe "tt"
const tooltips = document.querySelectorAll(".tt");
tooltips.forEach((t) => {
  new bootstrap.Tooltip(t);
});

// Une fonction qui prend un nom de classe et un "état" bool pour masquer/afficher les balises
...
function addWaterMark() {
    ...
    if (!watermark_file || !watermark_opacity || !watermark_position) {
        console.log("Outil Watermark: entrée invalide");
...
function addText() {
    ...
    if (!text_string || !text_font || !text_size || !text_color || !text_position) {
        console.log("Outil Texte: entrée invalide");
...
function rotateImg() {
    ...
    // validate input
    if (!degree) {
        console.log("Outil Rotation: entrée invalide");
...
function cropImg() {
    ...
    if (!left || !top || !right || !bottom) {
        console.log("Outil Recadrage: entrée invalide");
