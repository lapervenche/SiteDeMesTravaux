...
function onBruteForce() {
  if (!("importKey" in window.crypto.subtle)) {
    error("window.crypto non chargé. Veuillez recharger sur https");
    return;
  }
  ...
  if (charset == "") {
    error("Charset ne peut pas être vide.");
    return;
  }
  ...
  function progressUpdate() {
    ...
    error(`En essayant ${progress.total} mots de passe d\'une longueur de ${progress.len} caractères – Pourcentage de recherche ${Math.round(100000 * progress.tried / progress.total)/1000}% réalisé. Essai de ${Math.round(1000000 * (progress.overallTotal + progress.tried) / delta)/1000} mots de passe par seconde.`);
  }
